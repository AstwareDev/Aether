...
export default function Workspace({ path, onClose, onChangeWorkspace }: WorkspaceProps) {
  ...
  const commitCreate = useCallback(
    ...
    [creating, openFile, refreshTree, reloadIndex, notify],
  );
  ...
  const commitRename = useCallback(
    ...
    [renamingPath, remapPaths, refreshTree, reloadIndex, notify],
  );
  ...
  }, [deletingPath, remapPaths, refreshTree, reloadIndex, notify]);
  ...
  const moveEntry = useCallback(
    ...
    [remapPaths, refreshTree, reloadIndex, notify],
  );
  ...
  const commands: Command[] = useMemo(() => {
    const list: Command[] = [
      { id: "file.newFile", title: "New File", category: "File", icon: NewFileIcon, keywords: "create", run: () => beginCreate(path, false) },
      { id: "file.newFolder", title: "New Folder", category: "File", icon: NewFolderIcon, keywords: "create directory", run: () => beginCreate(path, true) },
  ...
  const tabs: OpenTab[] = useMemo(() => {
    const next = openPaths.map((p) => ({
      path: p,
      dirty: isDiffPath(p) ? false : buffers[p] ? buffers[p].value !== buffers[p].saved : false,
    }));
    const prev = prevTabsRef.current;
    const same =
      prev.length === next.length && prev.every((t, i) => t.path === next[i].path && t.dirty === next[i].dirty);
    if (same) return prev;
  ...
  return (
    <div className="flex h-full flex-col bg-canvas text-zinc-200">
      ...
      <StatusBar
        cursor={activePath && !activeBuffer?.error ? cursor : null}
        language={activePath ? languageLabelForPath(activePath) : null}
